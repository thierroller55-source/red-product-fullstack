...
exports.getNotifications = async (req, res) => {
  try {
    ...
    const notifs = await Notification.find(query).populate('owner', 'nom').sort({ date: -1 }).limit(10);
    res.json(notifs);
  } catch (error) {
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    // On tente d'envoyer le mail mais on ne fait pas planter le serveur si ça échoue
    try {
      ...
      console.log("Erreur mail (mais token créé) :", token);
      res.status(200).json({ success: true, message: "Accès passé (Mail en attente)", debugToken: token });
    }
