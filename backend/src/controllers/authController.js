...
exports.login = async (req, res) => {
  try {
    ...
    if (!user) return res.status(401).json({ success: false, message: "Accès refusé : Identifiants incorrects" });

    const isMatch = await bcrypt.compare(password, user.password);
    if (!isMatch) return res.status(401).json({ success: false, message: "Accès refusé : Identifiants incorrects" });

    // On génère le badge (Token) avec ID et RÔLE
...
exports.getNotifications = async (req, res) => {
  try {
    ...
    if (req.user.role === 'admin') query = {}; // L'admin voit tout
...
// ── 4. MOT DE PASSE OUBLIÉ (GMAIL RÉEL) ───────────────────────
exports.forgotPassword = async (req, res) => {
  try {
    ...
    try {
      ...
      console.log("Erreur mail réelle :", mailErr);
