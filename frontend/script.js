...
// ============================================================
// 3. AUTHENTIFICATION & NOTIFICATIONS
// ============================================================
...
async function chargerNotifications() {
    ...
    if (!notifMenu) return;
    try {
        const res = await fetch(`${API_HOTELS}/notifications`, { headers: { 'Authorization': `Bearer ${getToken()}` } });
        const data = await res.json();
        ...
        if (data.length === 0) { html += '<p class="p-4 text-[10px] text-gray-400 text-center italic">Aucun message</p>'; }
        else {
            data.forEach(n => {
